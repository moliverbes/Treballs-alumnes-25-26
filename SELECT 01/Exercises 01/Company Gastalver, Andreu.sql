-- EXERCICIS ANDREU COMPANY GASTALVER / 29/09/2025

-- 1. Mostra totes les columnes de totes les pel·lícules
SELECT * FROM film;

-- 2. Mostra id, títol i descripció de les pel·lícules amb rating PG-13
SELECT film_id, title, description, rating FROM film WHERE rating LIKE "PG-13";
...
SELECT title, rating FROM film WHERE rating LIKE "PG";
...
SELECT title, length, rating FROM film WHERE length > 120 AND rating LIKE "R";

-- 17. Mostra els títols de les pel·lícules amb el terme 'action' o 'comedy' en la descripció.
SELECT title, description FROM film WHERE description LIKE "%action%" OR description LIKE "%comedy%";
...
SELECT title, release_year, rating FROM film WHERE release_year > 2000 AND rating LIKE "PG" OR rating LIKE "G";