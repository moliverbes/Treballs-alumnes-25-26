...
SELECT rating, MIN(length) AS 'durada_minima', MAX(length) AS 'durada maxima'
FROM film
GROUP BY rating
HAVING MIN(length) < 90 AND MAX(length) > 150;
...
SELECT rating, AVG(replacement_cost), COUNT(film_id) 
FROM film
GROUP by rating
...
SELECT customer_id, MIN(amount), MAX(amount), payment_id 
FROM payment
GROUP BY payment_id
HAVING payment_id > 5 AND (MAX(amount) - MIN(amount)) > 5;
...
SELECT customer_id, rental_id, MIN(retanl_date) 
FROM rental
GROUP BY rental_id
HAVING rental_id > 8 AND MIN(return_date) < '2006-01-01';


-- Mostra les pel·lícules, el nombre de vegades que apareixen a la taula inventory i el cost total de reemplaçament estimat, considerant que cada còpia té un cost constant de 20 $, 
-- però només d’aquelles pel·lícules amb més de 4 còpies i un cost total superior a 100 $
...
SELECT customer_id  AS 'client_id', COUNT(payment_id) as 'pagament', AVG(amount) AS 'import'
FROM payment
GROUP BY client_id
HAVING pagament > 10
AND import < 3;
