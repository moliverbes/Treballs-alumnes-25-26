...
SELECT * FROM film WHERE rating NOT LIKE "PG-13";
...
SELECT * FROM actor WHERE first_name LIKE "%LL%";
...
SELECT * FROM film WHERE rating IN ('PG', 'R', 'NC-17');
...
SELECT title, length(title) AS title FROM film;

-- 18. Mostra els 3 primers caràcters del títol de cada pel·lícula.
SELECT title, SUBSTRING(title,1,3) FROM film;

-- 19. Mostra els noms i cognoms dels actors concatenats amb un espai al mig.
SELECT CONCAT_WS(' ',first_name, last_name) AS "Nom complet" FROM actor;