...
SELECT customer_id, COUNT(payment_id) AS 'TOTAL PAGAMENT', SUM(amount) AS 'TOTAL PAGAT'
FROM payment
GROUP BY customer_id
HAVING COUNT(payment_id) > 12 AND SUM(amount) > 50;

-- 2. Mostra la durada màxima i la durada mínima registrada per cada classificació (rating),
-- però només d’aquelles pel·lícules que tinguin una durada màxima superior a 150 minuts i una mínima inferior a 90.
SELECT rating, min(length) AS 'DURADA MIN', max(length) AS 'DURADA MAX'
FROM film
GROUP BY rating
...
FROM payment
GROUP BY staff_id
HAVING COUNT(payment_id) AND MAX(amount) > 8;
...
SELECT rating, AVG(replacement_cost) AS 'MITJANA REPLAÇAMENT', COUNT(film_id) AS 'SUMA PELÍCULES'
FROM film 
GROUP BY rating
...
SELECT film_id, 
	...
	20*COUNT(film_id) AS 'Cost_remplaçament'
FROM inventory
GROUP BY film_id
HAVING COUNT(film_id) > 4
AND Cost_remplaçament > 100;

-- 8. Mostra els clients, el nombre de vegades que han llogat pel·lícules i la data més recent de lloguer, però només dels que tinguin més de 8 lloguers
-- i una data de lloguer màxima anterior a l’any 2006.
SELECT customer_id,
COUNT(rental_id) AS 'Total_llogat',
MAX(rental_date) AS 'Data_recent'
FROM rental
GROUP BY customer_id
HAVING Total_llogat > 8 
AND MAX(rental_date) < '2006-01-01';
...
SELECT rating, 
AVG(rental_rate) AS 'MITJA_LLOGUER',
COUNT(film_id) AS 'TOTAL_PELIS'
FROM film
GROUP BY rating
HAVING TOTAL_PELIS > 10 
AND MITJA_LLOGUER < 3;

-- 10. Mostra els clients (client_id), el nombre total de pagaments i l’import mitjà pagat, però només aquells clients amb més de 10 pagaments
-- i amb un import mitjà inferior a 3 dòlars.
SELECT customer_id AS 'CLIENT_ID', 
COUNT(rental_id) AS 'TOTAL_PAGAMENTS',
AVG(amount) AS 'MITJA_PAGAT'
FROM payment
GROUP BY customer_id
