...
-- 8. Mostra per a cada línia de detall_comanda l’identificador de comanda, el codi de producte, la quantitat,
-- el preu unitari i una columna addicional amb el subtotal de la línia; ordena per identificador de comanda i després pel codi de producte.
...
SELECT id_producte, UPPER(nom) AS nom_majúscules, LENGTH(nom) AS llargada_nom FROM productes
ORDER BY llargada_nom DESC, nom_majúscules;
...
SELECT id_producte, nom, preu, ROUND(preu*1.21,2) AS nou_preu, stock FROM productes
WHERE stock >= 10
ORDER BY nou_preu DESC;
...
SELECT id_producte, nom, preu, ROUND(preu) AS preu_enter, stock FROM productes
...
SELECT id_comanda, data_comanda, YEAR(data_comanda) AS any, MONTH(data_comanda) AS mes, DAY(data_comanda) AS dia, TIME(data_comanda) AS hora FROM comandes
ORDER BY data_comanda ASC;

-- 14. De comandes, mostra l’identificador i la data formatejada com YYYY-MM en una 
-- columna anomenada any_mes; ordena per any_mes i després per identificador.
...
SELECT estat, AVG(import_total) AS import_mig, MAX(import_total) AS import_màxim, MIN(import_total) AS import_mínim FROM comandes
GROUP BY estat
ORDER BY estat ASC;
...
SELECT DATE_FORMAT(data_comanda,"%Y-%m") AS any_mes, COUNT(id_comanda) AS nombre_comandes, SUM(import_total) AS suma_imports, ROUND(AVG(import_total),2) AS import_mitjà FROM comandes
GROUP BY any_mes
...
-- excloent les categories ‘Accessoris’ i ‘Llibres’. Mostra només les categories que tinguin més d’un producte i ordena pel preu mitjà de més alt a més baix.
SELECT categoria, COUNT(id_producte) AS nombre_productes, AVG(preu) AS preu_mitjà FROM productes
WHERE categoria NOT IN ("Accessoris","Llibres")
GROUP BY categoria
HAVING nombre_productes > 1
ORDER BY preu_mitjà DESC;