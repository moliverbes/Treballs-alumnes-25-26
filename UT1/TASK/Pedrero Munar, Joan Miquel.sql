...
SELECT id_client AS "Identificaor", concat_ws(nom, cognoms) AS "Nom_Complet" FROM clients
...
SELECT id_producte AS "Identificador", nom AS "NOM", preu AS "Preu" FROM productes
WHERE Preu > 40
ORDER BY Preu ASC;
...
SELECT id_producte AS "Identificdor",
nom AS "NOM",
categoria AS "Categoria"
FROM productes
...
SELECT id_comanda AS "Identificador",
data_comanda AS "Fecha_Comanda", 
estat
FROM comandes
ORDER BY Fecha_Comanda DESC;

-- 8. Mostra per a cada línia de detall_comanda l’identificador de comanda, el codi de producte, la quantitat, el preu unitari 
-- i una columna addicional amb el subtotal de la línia; ordena per identificador de comanda i després pel codi de producte.
SELECT id_comanda,
id_producte,
quantitat,
preu_unitari,
quantitat*preu_unitari AS "Subtotal" FROM detall_comanda
...
SELECT id_producte,
UPPER(nom) AS "NOM_MAJÚSCULES",
LENGTH(nom) AS "Llargada"
FROM productes
ORDER BY Llargada DESC, NOM_MAJÚSCULES ASC;
...
SELECT id_client AS "Identificador",
nom AS "NOM",
...
SELECT id_producte AS "Identificador", 
nom, 
preu, 
ROUND(preu*1.21, 2) AS "Preu_incrementat",
stock
FROM productes
WHERE stock = 10 or stock > 10
...
FROM comandes
ORDER BY any_mes DESC, identificador DESC;
...
SELECT id_producte AS "identificador",
preu,
ROUND(preu*1.21, 2) AS "preu_informat"
FROM productes;
...
SELECT COUNT(id_comanda) AS "comandes",
estat
FROM comandes
GROUP BY estat
ORDER BY comandes ASC;

-- 18. De comandes, calcula l’import mig, el mínim i el màxim agrupant per estat; ordena per estat ascendent.
SELECT estat,
ROUND(AVG(import_total),2) AS "Import Mitja",
MIN(import_total) AS "Import Minim",
MAX(import_total) AS "Import Maxim"
FROM comandes
GROUP BY estat
ORDER BY estat ASC;
...
COUNT(id_comanda) AS "Nombre Comandes",
SUM(import_total) AS "Suma Imports",
ROUND(AVG(import_total),2) AS "Import Mitja"
FROM comandes
GROUP BY any_mes
...
SELECT id_comanda AS "identificador_comanda", 
id_producte AS "codi_producte", 
quantitat, 
preu_unitari, 
quantitat * preu_unitari AS "subtotal"
FROM detall_comanda
...
SELECT
CURRENT_DATE() AS "Data actual",
CURRENT_TIME() AS "Hora actual",
CURRENT_TIMESTAMP() AS "Marca de temps actual";
