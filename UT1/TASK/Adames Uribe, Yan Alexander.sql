...
SELECT id_producte, UPPER(nom) AS nom_majúscules, LENGTH(nom) AS llargada_nom
FROM productes
...
SELECT id_producte, nom, preu, ROUND(preu*1.21,2) AS nou_preu, stock
FROM productes
WHERE stock >= 10
...
SELECT id_producte, nom, preu, ROUND(preu) AS preu_enter, stock
FROM productes
...
SELECT categoria, COUNT(id_producte) AS nombre_productes, AVG(preu) AS preu_mitjà
FROM productes
