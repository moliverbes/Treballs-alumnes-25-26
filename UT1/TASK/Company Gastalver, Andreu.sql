...
SELECT CONCAT_WS(" ",id_client, nom, cognoms) AS nom_complet
FROM clients 
ORDER BY nom_complet ASC;

-- 2 Llista dels productes amb tres columnes anomenades exactament NOM_PRODUCTE, CATEGORIA i PREU_EUR; ordena pel preu de més alt a més baix 
-- i limita el resultat a 5 files.
SELECT nom AS NOM_PRODUCTE, categoria AS CATEGORIA, preu AS PREU_EUR
FROM productes
...
SELECT id_producte AS Producte, nom AS Nom, preu AS Preu
FROM productes
WHERE preu > 40
ORDER BY preu ASC;
...
-- 8 Mostra per a cada línia de detall_comanda l’identificador de comanda, el codi de producte, la quantitat, el preu unitari i una columna 
-- addicional amb el subtotal de la línia; ordena per identificador de comanda i després pel codi de producte.
...
SELECT id_producte AS Identificador, UPPER(nom) AS Nom, length(nom) AS Llargaria
FROM productes
ORDER BY Llargaria DESC;
...
SELECT id_client AS Identificador_client, nom AS Nom, SUBSTRING(cognoms, 1, 5) AS Prefix_cognoms
FROM clients
...
SELECT id_producte AS identificador_prod, nom, preu, ROUND(preu * 1.21, 2) AS preu_iva, stock AS proveïments
FROM productes
WHERE stock >= 10 
ORDER BY preu DESC;
...
SELECT id_producte, nom, preu, ROUND(preu)
FROM productes
WHERE MOD(stock,2) NOT IN (0)
ORDER BY stock DESC;
...
SELECT id_comanda, data_comanda, YEAR(data_comanda) AS any, MONTH(data_comanda) AS mes, DAY(data_comanda) AS dia, TIME(data_comanda) AS hora
FROM comandes
ORDER BY data_comanda ASC;

-- 14 De comandes, mostra l’identificador i la data formatejada com YYYY-MM en una columna anomenada any_mes; ordena per any_mes i 
-- després per identificador.
...
SELECT estat, AVG(import_total) AS import_mig, MAX(import_total) AS import_màxim, MIN(import_total) AS import_mínim
FROM comandes
GROUP BY estat
ORDER BY estat ASC;

-- 19 De comandes, calcula per a cada any-mes (YYYY-MM) el nombre de comandes, la suma d’imports i l’import mitjà arrodonit a 2 decimals; 
-- ordena per any-mes ascendent.
SELECT DATE_FORMAT(data_comanda,"%Y-%m") AS any_mes, COUNT(id_comanda) AS nombre_comandes, SUM(import_total) AS suma_imports, ROUND(AVG(import_total),2) AS import_mitjà
FROM comandes
GROUP BY any_mes
ORDER BY any_mes ASC;

-- 20 De la taula comandes, mostra per a cada estat el nombre de comandes i l’import total acumulat de les comandes 
-- realitzades durant l’any 2024. Ordena de més a menys import total.
SELECT estat, COUNT(id_comanda) AS nombre_comandes, SUM(import_total) AS import_acumulat
FROM comandes
...
-- 25 De la taula productes, mostra per a cada categoria el nombre de productes i el preu mitjà, excloent les categories 
-- ‘Accessoris’ i ‘Llibres’. Mostra només les categories que tinguin més d’un producte i ordena pel preu mitjà de més alt a més baix.
SELECT COUNT(id_producte) AS Nom_prod, ROUND(AVG(preu),2) AS preu, categoria
FROM productes
WHERE categoria NOT IN ("Accessoris","Llibres")
GROUP by categoria
HAVING COUNT(id_producte) > 1
ORDER BY preu DESC;