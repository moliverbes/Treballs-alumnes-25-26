...
SELECT LOWER(title) AS 'title', rating FROM film
WHERE rating = 'PG-13';

-- 17. Mostra una columna amb la longitud (LENGTH) del títol de cada pel·lícula. --

SELECT title, length(title) AS 'num_letters_title' FROM film;

-- 18. Mostra els 3 primers caràcters del títol de cada pel·lícula. --

SELECT title, SUBSTRING(title, 1, 3) AS 'first_three_letters' FROM film;
