-- Arnau Garcia Franco UT8.CompulsoryTask.

-- Primera part Creacio Base de dades :
-- Creacio BSD
...
ADD CONSTRAINT chk_data_naixement_no_futura1 CHECK (data_naixement <= CURDATE);
...
CREATE TABLE IF NOT EXISTS cites (
							...
							CONSTRAINT chk_estat_valid CHECK (estat IN ('Pendent', 'Completada', 'Cancel·lada'))
    );
...
CREATE TABLE medicaments (
							...
							stock INT DEFAULT 0,
                            CONSTRAINT stock_negatiu CHECK  (stock > 0)
                            );
...
CREATE TABLE prescriptions ( 
                            ...
                            CONSTRAINT quantitat_sup CHECK  (quantitat > 0)
                            );
